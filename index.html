...
function createRing(x, y, color) {
    ...
    return {
        x: x, y: y, vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4,
        color: color, originalColor: color, landed: false, pegIndex: -1, landedOrder: -1,
        basePoints: 25, awardedPoints: 0,
...
function initPegs() {
    ...
    pegData.forEach((data, index) => {
        pegs.push({ 
            ...
            bottomY: gameScreenHeight - 20 - (gameScreenHeight * data.yOffsetFactor * (index > 1 ? 1 : 0)), 
...
function initGame() {
    ...
    for (let i = 0; i < RING_COLORS.length; i++) {
        for (let j = 0; j < MAX_RINGS_PER_PEG; j++) { 
...
function drawRing(ring) {
    ...
    if (ring.isFlat) {
        ...
        if (scaleYValue < 0.08 && !ring.landed) { 
             ...
             const flatDrawWidth = outerRadius * 2;
             ctx.fillStyle = RING_OUTLINE_COLOR;
             ctx.fillRect( -flatDrawWidth / 2 - RING_OUTLINE_WIDTH_ON_SCREEN, -halfFlatViewThickness - RING_OUTLINE_WIDTH_ON_SCREEN, flatDrawWidth + (RING_OUTLINE_WIDTH_ON_SCREEN * 2), tempFlatThickness + (RING_OUTLINE_WIDTH_ON_SCREEN * 2) );
             ctx.fillStyle = ring.color;
             ctx.fillRect( -flatDrawWidth / 2, -halfFlatViewThickness, flatDrawWidth, tempFlatThickness );
        } else {
...
function drawAllPegsAndLandedRings() {
    pegs.forEach(peg => {
        ctx.fillStyle = PEG_FILL_COLOR; ctx.strokeStyle = PEG_STROKE_COLOR; ctx.lineWidth = 2;
...
function drawScoreOnCanvas() {
    ...
    // No dibujar si estamos en la pantalla de inicio. Si es game over, el score final se muestra en el modal.
...
function showMessage(text, duration = 3000, isInstruction = false) {
    ...
    if (!isInstruction) {
        instructionTimeout = setTimeout(() => {
            ...
            setTimeout(setPersistentInstructions, 700); 
        }, duration);
    }
}

function setPersistentInstructions() { 
    if (instructionTimeout && messageBoard.textContent !== "" && !messageBoard.textContent.toLowerCase().includes("pc:")) {
        return; 
    }
    let instructionMessage = "PC: Flechas=Inclinar, A/D=Jets.";
    if (sensorAvailable) {
        if (sensorActive) instructionMessage += " Móvil: Sensor ACTIVO.";
        else instructionMessage += " Móvil: Botones/Activar Sensor.";
    } else {
        instructionMessage += " Móvil: Botones TILT.";
    }
...
function checkAndApplyBonuses(landedRing, peg) { 
    ...
    // --- NUEVO: Vibración al encestar un aro ---
    if ('vibrate' in navigator) {
        navigator.vibrate(75); // Vibra por 75 milisegundos
    }
    // --- FIN NUEVO ---
    ...
    if (peg.landedRings.length === MAX_RINGS_PER_PEG && !peg.isFullAndScored) {
        ...
        if (isCurrentPegMonoColor) {
            ...
            peg.landedRings.forEach(r => { pegTotalAwardedPoints += r.awardedPoints; });
...
function showEndGameScreen() {
    ...
    let summaryHTML = `<h2>¡Juego Terminado!</h2>`;
    summaryHTML += `<p>Puntos Base Aros: ${baseScoreFromRings}</p>`;
    ...
    playAgainButton.onclick = () => {
        ...
        howToPlayButton.style.display = 'inline-block';
        gameRunning = false;
        score = 0; 
        currentScoreDisplaySize = SCORE_NORMAL_SIZE; 
    };
...
function createFloatingScore(x, y, text, color = "#FFFFFF", durationFrames = 90, upwardSpeed = 0.8) { 
    floatingScores.push({
        x: x, y: y, text: text, color: color, opacity: 1, vy: -upwardSpeed, 
        life: durationFrames, initialLife: durationFrames, currentFontSize: 20, maxFontSize: 30    
    });
}
...
function updateAndDrawFloatingScores(dt) { 
    ...
    ctx.save();
    ctx.textAlign = "center"; ctx.shadowColor = "rgba(0,0,0,0.6)"; 
    ctx.shadowBlur = 4; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 2;
    for (let i = floatingScores.length - 1; i >= 0; i--) {
        const fs = floatingScores[i];
        fs.y += fs.vy * accelerationFactor; fs.life -= accelerationFactor;      
...
function updateAndDrawJetParticles(dt) {
    ...
    for (let i = jetParticles.length - 1; i >= 0; i--) {
        ...
        p.x += p.vx * accelerationFactor; p.y += p.vy * accelerationFactor;
        p.vy += GRAVITY_BASE * 0.2 * accelerationFactor; p.life -= accelerationFactor;
        ...
        if (p.opacity <= 0 || p.radius <= 0.4 || p.life <= 0 || p.y < -p.radius || p.y > gameScreenHeight + p.radius) {
            ...
            ctx.fillStyle = `rgba(${colorParts}, ${p.opacity.toFixed(2)})`; ctx.fill();
        }
    }
    ctx.restore();
}
...
function updateRings(actualTiltForceToApply, dt) { 
    ...
    rings.forEach(ring => { 
        if (ring.landed && !ring.isSlidingOnPeg) {
            ...
            ring.zRotationAngle = 0; ring.zRotationSpeed = 0; ring.vx = 0; ring.vy = 0;
            return;
        }
        if (ring.isSlidingOnPeg) {
            ...
            if (ring.y < ring.finalYonPeg) { 
                ...
                if (ring.y >= ring.finalYonPeg) {
                    ring.y = ring.finalYonPeg; ring.isSlidingOnPeg = false; ring.vy = 0; landedRingsCount++; 
                }
            } else { 
                ring.y -= slideSpeedValue;
                 if (ring.y <= ring.finalYonPeg) {
        ...
        if (actualTiltForceToApply !== 0) {
            ring.vx += actualTiltForceToApply * accelerationFactor; 
            forceAppliedToRingThisFrame = true;
        }
        const isOnGroundAndEffectivelySlow = ring.y + RING_OUTER_RADIUS >= gameScreenHeight - (GROUND_FLAT_RING_THICKNESS / 2 + RING_OUTLINE_WIDTH_ON_SCREEN + 1) &&
                                          Math.abs(ring.vy) < 0.15 && Math.abs(ring.vx) < 0.15; 
        if (ring.isFlat && forceAppliedToRingThisFrame && !isOnGroundAndEffectivelySlow) { 
            ring.isFlat = false;
            ...
            ring.isFlat = true; ring.zRotationSpeed = 0; 
        }
        if (!ring.isFlat) {
            ...
            if (Math.abs(ring.rotationSpeed) < 0.005 / (accelerationFactor > 0 ? accelerationFactor : 1) ) ring.rotationSpeed = 0;
             ...
             ring.rotationAngle = Math.PI / 2; ring.rotationSpeed = 0;
        }
        ring.zRotationAngle += ring.zRotationSpeed * accelerationFactor;
        if (ring.zRotationAngle > Math.PI * 2) ring.zRotationAngle -= Math.PI * 2; else if (ring.zRotationAngle < 0) ring.zRotationAngle += Math.PI * 2;
    ...
    for (let iter = 0; iter < 3; iter++) { 
        for (let i = 0; i < rings.length; i++) {
            const ring1 = rings[i];
            if (ring1.landed) continue;
            for (let j = i + 1; j < rings.length; j++) {
                const ring2 = rings[j];
                if (ring2.landed) continue;
    ...
    rings.forEach((ring, index) => { 
        if (ring.landed && !ring.isSlidingOnPeg) return; 
        if (ring.isSlidingOnPeg) return;
        let prevX = ring.x; let prevY = ring.y; 
        ...
        if (!ring.landed) { 
            for (const peg of pegs) { 
                ...
                const ringRadius = RING_OUTER_RADIUS; const landingCatchWidth = PEG_VISUAL_WIDTH * PEG_LANDING_WIDTH_FACTOR; 
                const horizontallyAligned = Math.abs(ring.x - pegCenterX) < landingCatchWidth / 2;
                const isFalling = ring.vy > 0; 
                const ringBottom = ring.y + ringRadius; const prevRingBottom = prevY + ringRadius;
                if (isFalling && horizontallyAligned && 
                    ringBottom >= pegTop && prevRingBottom < pegTop + RING_VISUAL_THICKNESS * 0.8 ) { 
                    const targetLandedY = (peg.bottomY - FLAT_RING_VIEW_THICKNESS / 2) - (peg.landedRings.length * FLAT_RING_VIEW_THICKNESS);
                    ring.isSlidingOnPeg = true; ring.finalYonPeg = targetLandedY; ring.landed = true; 
                    ring.isFlat = true; ring.pegIndex = peg.id; ring.x = pegCenterX; 
                    if (ring.y < targetLandedY) { ring.vy = Math.min(4.0, 2.0 + peg.landedRings.length * 0.1); } 
                    else { ring.vy = -Math.min(4.0, 2.0 + peg.landedRings.length * 0.1); 
                    ...
                    ring.landedOrder = peg.landedRings.length; peg.landedRings.push(ring); 
                    checkAndApplyBonuses(ring, peg); interactionOccurredThisFrame = true; break; 
                }
                if (!ring.landed && !interactionOccurredThisFrame) {
                    const pegBodyLeft = pegCenterX - (PEG_VISUAL_WIDTH * PEG_COLLISION_WIDTH_FACTOR) / 2;
                    const pegBodyRight = pegCenterX + (PEG_VISUAL_WIDTH * PEG_COLLISION_WIDTH_FACTOR) / 2;
                    if (ring.y + ringRadius > pegTop + RING_VISUAL_THICKNESS * 0.5 && ring.y - ringRadius < peg.bottomY) {
                        if (ring.x + ringRadius > pegBodyLeft && prevX + ringRadius <= pegBodyLeft + 1 && ring.vx > 0) { 
                            ring.x = pegBodyLeft - ringRadius - 0.1; ring.vx *= PEG_COLLISION_BOUNCE_FACTOR; interactionOccurredThisFrame = true;
                        } else if (ring.x - ringRadius < pegBodyRight && prevX - ringRadius >= pegBodyRight -1 && ring.vx < 0) {
                            ring.x = pegBodyRight + ringRadius + 0.1; ring.vx *= PEG_COLLISION_BOUNCE_FACTOR; interactionOccurredThisFrame = true;
                        }
                    }
                    if (!interactionOccurredThisFrame && isFalling &&
                        ring.y + ringRadius > pegTop && prevY + ringRadius <= pegTop + 3 && 
                        Math.abs(ring.x - pegCenterX) < (PEG_VISUAL_WIDTH / 2 + ringRadius)) { 
                            ring.y = pegTop - ringRadius - 0.1; 
        ...
        if (ring.x - RING_OUTER_RADIUS < 0) { ring.x = RING_OUTER_RADIUS; ring.vx *= BOUNCE_FACTOR; }
        if (ring.x + RING_OUTER_RADIUS > gameScreenWidth) { ring.x = gameScreenWidth - RING_OUTER_RADIUS; ring.vx *= BOUNCE_FACTOR; }
        if (ring.y - RING_OUTER_RADIUS < 0) { ring.y = RING_OUTER_RADIUS; ring.vy *= BOUNCE_FACTOR; }
        const effectiveRingBottomExtent = ring.y + (ring.isFlat ? GROUND_FLAT_RING_THICKNESS / 2 : RING_OUTER_RADIUS);
        const groundHitPosition = gameScreenHeight - RING_OUTLINE_WIDTH_ON_SCREEN; 
        if (effectiveRingBottomExtent >= groundHitPosition) { 
            ring.y = groundHitPosition - (ring.isFlat ? GROUND_FLAT_RING_THICKNESS / 2 : RING_OUTER_RADIUS); 
...
leftJetButton.addEventListener('mousedown', () => leftJetActive = true);
leftJetButton.addEventListener('mouseup', () => leftJetActive = false);
leftJetButton.addEventListener('mouseleave', () => { if(leftJetActive) leftJetActive = false; });
leftJetButton.addEventListener('touchstart', (e) => { e.preventDefault(); leftJetActive = true; }, { passive: false });
leftJetButton.addEventListener('touchend', (e) => { e.preventDefault(); leftJetActive = false; });
rightJetButton.addEventListener('mousedown', () => rightJetActive = true);
rightJetButton.addEventListener('mouseup', () => rightJetActive = false);
rightJetButton.addEventListener('mouseleave', () => { if(rightJetActive) rightJetActive = false; });
rightJetButton.addEventListener('touchstart', (e) => { e.preventDefault(); rightJetActive = true; }, { passive: false });
rightJetButton.addEventListener('touchend', (e) => { e.preventDefault(); rightJetActive = false; });
...
window.addEventListener('click', (event) => {
    if (event.target == howToPlayScreen) {
        howToPlayScreen.style.display = 'none';
    }
});
...
window.addEventListener('keydown', (e) => {
    ...
    switch (e.code) {
        ...
        case KEY_JET_LEFT: leftJetActive = true; keyProcessed = true; break;
        case KEY_JET_RIGHT: rightJetActive = true; keyProcessed = true; break;
    }
...
window.addEventListener('keyup', (e) => {
    ...
    switch (e.code) {
        ...
        case KEY_JET_LEFT: leftJetActive = false; keyProcessed = true; break;
        case KEY_JET_RIGHT: rightJetActive = false; keyProcessed = true; break;
    }
     if (keyProcessed && (gameRunning || startScreen.style.display === 'none')) e.preventDefault();
});
...
function requestSensorPermission() {
    ...
    setTimeout(() => {
        enableSensorButton.classList.remove('sensor-button--activating');
    }, 300);
    ...
    if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        DeviceOrientationEvent.requestPermission()
            .then(permissionState => {
                if (permissionState === 'granted') {
                    ...
                    tiltLeftButton.style.display = 'none'; 
                    tiltRightButton.style.display = 'none';
                    setPersistentInstructions(); 
                } else {
                    showMessage("Permiso del sensor denegado.", 3000, true);
                     enableSensorButton.textContent = "Sensor Denegado";
                     enableSensorButton.disabled = true;
                }
            })
            .catch(error => {
                console.error("Error al solicitar permiso del sensor:", error);
                showMessage("Error al activar sensor.", 3000, true);
            });
        ...
        showMessage("Sensor no soportado.", 3000, true);
        enableSensorButton.style.display = 'none';
    }
}
...
if (window.DeviceOrientationEvent) {
    ...
    console.log("Sensor de orientación no soportado por este navegador/dispositivo.");
    enableSensorButton.style.display = 'none';
}
